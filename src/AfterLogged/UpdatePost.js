...
const fetchResourceFromURI = async uri => {
  ...
  const updatePost = () => {
    fetch('http://localhost:3000/updatePost',
    {
        ...
        body:JSON.stringify({
            ...
            body:body ? body : prevPost.body,
            image:image ? image : prevPost.image,
            image2:image2 ? image2 : prevPost.image2,
            date,
            postId:prevPost._id
        })
    })
  ...
  return (
    <View style={styles.container}>
        ...
        <TouchableHighlight onPress={!prevPost ? createNewPost : updatePost} style={styles.btn}><Text style={styles.textBtn}>
              {prevPost ? 'Update':'Create'}
              </Text></TouchableHighlight>
        </View>
        
       {!prevPost ? <View style={{flexDirection:'row'}}>
        ...
        {image ? <Pressable onPress={() => setImage(null)}><Image source={{ uri: image }} style={{ width: 210, height: 200}} /></Pressable> : <Pressable onPress={pickImage}><Image source={{ uri: prevPost.image }} style={{ width: 210, height: 200}} /></Pressable>}
        {image2 ? <Pressable onPress={() => setImage2(null)}><Image  source={{ uri: image2 }} style={{ width:210, height: 200}} /></Pressable> : <Pressable onPress={pickImage}><Image source={{ uri: prevPost.image2 }} style={{ width: 210, height: 200}} /></Pressable>}
        {isLoading && <ActivityIndicator size={'large'} color={'blue'} />}
        </View>
        }
        <TextInput 
